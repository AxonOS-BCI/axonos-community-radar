...
def find_alert():
    """The existing alert issue, None if there is provably none, "RETRY" if we
    could not look.

    Same discipline as the stats digest: the only safe reason to open a new
    alert is a *successful* listing that found no marker. Treating a 403
    (rate-limited / under-scoped token) or a malformed response as "no alert
    exists" makes every failed lookup open a duplicate alert — and a monitor
    that spams duplicates is a monitor people mute.
    """
...
def main():
    ...
    if alert == "RETRY":
        # We could not read the issue list, so we cannot know whether an alert
        # already exists. Opening one would duplicate; closing one would be
        # blind. Skip: the next run (twice daily) re-checks. The diagnosis
        # itself is unaffected — only the bookkeeping is deferred.
        print("health: cannot enumerate issues; skipping to avoid duplicate alerts")
        print("STATUS:", "; ".join(problems) if problems else "healthy")
        return 0
